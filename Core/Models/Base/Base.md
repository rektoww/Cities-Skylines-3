...
`Building` - это абстрактный класс, наследуемый от `GameObject` и реализующий интерфейс `IConstructable`. Он служит базой для всех зданий в игре и определяет общие свойства, логику размещения на карте и требования для строительства.

### Свойства для строительства (IConstructable)

Эти статические поля определяют "рецепт" постройки для каждого дочернего класса здания.

- **`BuildCost`**: `static decimal` - Стоимость постройки здания.
- **`RequiredMaterials`**: `static Dictionary<ConstructionMaterial, int>` - Словарь с материалами, необходимыми для постройки.
