...
`CommercialBuilding` представляет собой базовый класс для всех коммерческих зданий. Наследуется от `Building` и предоставляет общую функциональность для магазинов, кафе, заправок и других коммерческих заведений.

### Свойства коммерции

- **`Type`**: `CommercialBuildingType` - Тип коммерческого здания (`Shop`, `Supermarket`, `Cafe`, `Restaurant`, `GasStation`, `Pharmacy`)
- **`Capacity`**: `int` - Вместимость здания (максимальное количество посетителей)
- **`EmployeeCount`**: `int` - Количество сотрудников
- **`ProductCategories`**: `List<string>` - Категории товаров/услуг, предоставляемых заведением
...
### Конкретные классы коммерческих зданий

- **`Shop`** - Магазин: продукты, напитки
- **`Supermarket`** - Супермаркет: продукты, напитки, хозяйственные товары  
- **`Cafe`** - Кафе: еда, напитки, десерты
- **`Restaurant`** - Ресторан: еда, напитки, десерты, алкоголь
- **`GasStation`** - Заправка: бензин, дизель, сопутствующие товары
- **`Pharmacy`** - Аптека: лекарства, медицинские товары, витамины

**Примечание:** Каждый тип коммерческого здания имеет свои дефолтные значения вместимости, количества сотрудников, стоимости строительства и категорий товаров.
