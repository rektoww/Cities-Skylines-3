...
- **`TreeType`**: `TreeType?` - Тип дерева на тайле (null если нет деревьев)
- **`TreeCount`**: `int` - Количество деревьев на тайле (0-10)
- **`HasForest`**: `bool` (только для чтения) - Является ли тайл лесом
