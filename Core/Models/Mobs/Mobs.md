...
`Airplane` — конкретная реализация `CommercialTransport`, представляющая **самолёт**, перемещающий товары между воздушными портами.

### Свойства

* **`DeparturePort`**: `Port` — Порт отправления.
* **`DestinationPort`**: `Port` — Порт назначения.
* **`HasReachedDestination`**: `bool` — Флаг, указывающий, достиг ли самолёт конечной точки маршрута.

### Методы

* **`Move()`**: `override void` — Реализует передвижение по прямой между портами отправления и назначения.
* **`CheckDestinationReached()`**: `bool` — Проверяет, достиг ли самолёт порта назначения и обновляет флаг прибытия.

---

## Глава 6: Ship.cs

`Ship` — конкретная реализация `CommercialTransport`, описывающая **корабль**, курсирующий между морскими портами и перевозящий ресурсы.

### Свойства

* **`DeparturePort`**: `Port` — Порт отправления.
* **`DestinationPort`**: `Port` — Порт назначения.
* **`HasReachedDestination`**: `bool` — Флаг достижения конечного порта.

### Методы

* **`Move()`**: `override void` — Реализует временную логику передвижения по прямой между портами (заглушка до добавления маршрутизации).
* **`CheckDestinationReached()`**: `bool` — Проверяет, достиг ли корабль порта назначения и устанавливает флаг `HasReachedDestination`.