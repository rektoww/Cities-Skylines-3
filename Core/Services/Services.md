...
- Класс получает в конструкторе ссылки на инвентарь материалов и баланс игрока.
- Метод `TryBuild<T>` является универсальным строителем для любого здания, наследующего `Building` и `IConstructable`.
...
- **`TryBuild<T>(...)`**: `bool`
  ...
  - Проверяет, можно ли разместить здание на карте.
  - Если все проверки пройдены, вычитает ресурсы, создает экземпляр здания с помощью `Activator.CreateInstance`, размещает его на карте и возвращает `true`.
...
- **`GetUtilityStatistics()`**: `void` - Собирает статистику по всем подключенным зданиям (в текущей реализации выводит в консоль).
